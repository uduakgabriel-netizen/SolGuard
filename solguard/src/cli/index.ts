...
import { AttestationService } from '../core/attestation/service';
import { Attestor } from '../core/attestation/attestor';
...
attest
  ...
  .action(async (options) => {
    ...
    const dbPath = path.join(process.cwd(), `kora-rent-${options.network}.db`);
    const db = AppDatabase.getInstance(dbPath);
    ...
    const service = new AttestationService(db, options.network);
...
attest
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const isValid = Attestor.verify(doc);
