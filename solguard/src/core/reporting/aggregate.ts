...
import { LifecycleState } from '../../db/database';

export interface GlobalMetrics {
    total_discovered: number;
    total_evaluated: number; // Basically any account not in DISCOVERED state? Or explicit check?
    // Let's assume evaluated means we have at least checked it once, so state != DISCOVERED or last_lifecycle_check is not null.
    // Or we can just sum up the states.
    state_counts: Record<LifecycleState | string, number>;
    total_reclaimed_lamports: number;
    total_failed_reclamations: number;
    average_batch_size: number; // Hard to calculate from current schema unless we trace transactions or have batch table. 
    // We will estimate or skip if data not present. Alternatively, we can average count of events in successful reclaims if verified.
    // Actually, we can check 'lifecycle_events' for 'RECLAIMED' and see if we can group by timestamp/signature.
    execution_duration_sec: number; // Time from first discovery to last event?
}
...
export class MetricsAggregator {
    ...
    public getGlobalMetrics(): GlobalMetrics {
        // 1. Total Discovered
        const totalDiscovered = this.db.prepare(
            `SELECT COUNT(*) as count FROM sponsored_accounts`
        ).pluck().get() as number;

        // 2. State Breakdown
        const stateRows = this.db.prepare(
            `SELECT lifecycle_state, COUNT(*) as count FROM sponsored_accounts GROUP BY lifecycle_state`
        ).all() as { lifecycle_state: string; count: number }[];
        ...
        const reclaimEvents = this.db.prepare(
            `SELECT evidence_payload FROM lifecycle_events WHERE new_state = 'RECLAIMED'`
        ).all() as { evidence_payload: string }[];
        ...
        const failedCount = this.db.prepare(
            `SELECT COUNT(*) as count FROM lifecycle_events WHERE new_state = 'FAILED'`
        ).pluck().get() as number;

        // 5. Execution Duration
        // Min discovered_at vs Max timestamp in events
        const firstDiscovery = this.db.prepare(
            `SELECT MIN(discovered_at) FROM sponsored_accounts`
        ).pluck().get() as string;

        const lastEvent = this.db.prepare(
            `SELECT MAX(timestamp) FROM lifecycle_events`
        ).pluck().get() as string;
