...
export type LifecycleState = 'DISCOVERED' | 'ACTIVE' | 'DORMANT' | 'CLOSED' | 'RECLAIMABLE' | 'PROTECTED' | 'DUST' | 'SKIPPED' | 'RECLAIMED' | 'FAILED' | 'closed_zero';
...
export class AppDatabase {
  ...
  public static getInstance(dbPath: string, verbose: boolean = false): Database.Database {
    if (!AppDatabase.instance) {
      console.log(`[DB] Initializing database at: ${dbPath}`);
      AppDatabase.instance = new Database(dbPath, { verbose: verbose ? console.log : undefined });
   ...
   */
  public static log(db: Database.Database, level: LogLevel, message: string, metadata?: object): void {
    try {
      db.prepare(
        'INSERT INTO audit_log (level, message, metadata) VALUES (?, ?, ?)'
